...
public class PilotNew {
    ...
    public static void test() throws IOException, IloException {
        ...
        for (int i = 0; i < listaGrafi.size(); i++) {
            ...
            //HashMap<Integer, Integer> pesoArchi = CalcolaPesoArchi(g, fr);
            int sommaFr = 0;
            ...
            /*Pilot p = new Pilot(g);
            colorsToDelete.add(keys.get(keys.size() - 1));
            colorsToDelete.add(keys.get(keys.size() - 2));
            sol = p.esegui(colorsToDelete);
            colorsToDelete.clear();
            System.out.println("Grafo:" + listaGrafi.get(i) + " Sol:" + sol);
            System.out.println("");
            xls.addInfoGrafo(listaGrafi.get(i), "spilot", 0, sol);
            ...
            xls.addInfoGrafo(listaGrafi.get(i), "pilot", endTime, minimumSolution);
            xls.salva(pathTabellaRisultati);
        }
