...
public class TestGreedy {
    ...
    public static void main() throws IOException {
        try (PrintWriter writer = new PrintWriter("src\\Risultati\\RisultatiGreedy.txt")) {
            ...
            for (String s : listaGrafi) {
                ...
                //GrafoColorato mlst = new Greedy(grafo, writer).esegui();
                Greedy greedy = new Greedy(grafo);
                GrafoColorato mlst = greedy.esegui();

                /**
                 * Stampo le statistiche dell'esecuzione del greedy
                 */
                Statistiche statistiche = greedy.getStatistiche();

                writer.printf("Tempo di esecuzione: %.3fs\n", statistiche.tempoEsecuzione);
                writer.println("Numero Iterate: " + statistiche.iter);
                writer.println("Colori usati: " + mlst.getListaColori().size());
                writer.printf("Media tempo iterata: %.3fs\n", (float) statistiche.tempoMedioIterate / 1000);
                writer.printf("Media determinazione colore più ricorrente: %.3fs\n", statistiche.tempoMedioDeterminazioneColorePiuRicorrente / 1000);
                writer.printf("Media inserimento archi senza ciclo: %.3fs\n", (float) statistiche.tempoMedioInserimentoArchiSenzaCiclo / 1000);
                writer.println();
                System.out.printf("Tempo di esecuzione: %.3fs\n", statistiche.tempoEsecuzione);
                System.out.println("Numero Iterate: " + statistiche.iter);
                System.out.println("Colori usati: " + mlst.getListaColori().size());
                System.out.printf("Media tempo iterata: %.3fs\n", (float) statistiche.tempoMedioIterate / 1000);
                System.out.printf("Media determinazione colore più ricorrente: %.3fs\n", statistiche.tempoMedioDeterminazioneColorePiuRicorrente / 1000);
                System.out.printf("Media inserimento archi senza ciclo: %.3fs\n", (float) statistiche.tempoMedioInserimentoArchiSenzaCiclo / 1000);
                writer.println();
